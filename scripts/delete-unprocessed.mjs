...
const unproc = db.prepare("SELECT video_id FROM signals WHERE processing_state = 'pending'").all();
...
// Delete the signals
const result = db.prepare("DELETE FROM signals WHERE processing_state = 'pending'").run();
console.log('Deleted', result.changes, 'signals');

// Verify
const remaining = db.prepare("SELECT COUNT(*) as cnt FROM signals WHERE processing_state = 'pending'").get();
