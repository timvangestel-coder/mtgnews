...
describe('PollRunManager streaming pipeline (issue #80)', () => {
  ...
  // ── Done counter incremented on failure ──

  describe('done counter', () => {
    it('increments done counter even when analysis fails', async () => {
      // Add a topic and channel
      db.prepare("INSERT INTO topics (id, key, short_name, filter_text) VALUES (?, ?, ?, ?)").run(1, 'tech', 'Tech', 'technology');
      db.prepare(
        "INSERT INTO channels (channel_id, display_name, active, added_at, topic_id) VALUES (?, ?, 1, ?, ?)"
      ...
      // Mock pollChannel to insert a signal, then mock analyzeSignal to fail
      vi.mock('./poll', async (importOriginal) => {
        const actual = await importOriginal<typeof import('./poll')>();
        return {
          ...actual,
          pollChannel: vi.fn().mockImplementation(async (database, channelId) => {
            if (channelId === 'UC_fail') {
              ...
              ).run('fail_vid1', channelId, 'Fail Video', new Date().toISOString(), JSON.stringify([{ start: 0, text: 'test' }]), Date.now(), 1);
      ...
      // Re-import after mocking so the mocked dependencies are used
      const { PollRunManager: PRM } = await import('./poll-run-manager');
  ...
  describe('abort', () => {
    ...
    it('deletes unsummarized signals on abort', async () => {
      ...
      // Insert a processed signal (should NOT be deleted)
      db.prepare(
        "INSERT INTO signals (video_id, channel_id, title, published_at, transcription, created_at, poll_run_id, processed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
      ).run('abort_vid2', 'UC_abort_test', 'Processed Video', new Date().toISOString(), JSON.stringify([{ start: 0, text: 'test' }]), Date.now(), runId, Date.now());
